 ...
 * can be created using {@link UnsupervisedScope#forkUnsupervised} or {@link UnsupervisedScope#forkCancellable}.
 *
...
public abstract class UnsupervisedScope {
     ...
     * <p>
     * Implementation note: a cancellable fork is created by starting a nested scope in a fork, and then starting a fork
     * there. Hence, it is more expensive than {@link Scope#fork}, as two virtual threads are started.
     */
    public <T> CancellableFork<T> forkCancellable(Callable<T> f) {
        ...
        getScope().fork(() -> {
            ...
            scopedWithCapability(nestedCapability, cap2 -> {
                nestedCapability.getScope().fork(() -> {
