...
public class Actor<T> {
    ...
    private final Sink<Consumer<T>> c;

    public Actor(Sink<Consumer<T>> c) {
        this.c = c;
    }
     ...
     */
    public <U> U ask(Function<T, U> f) throws Exception {
     ...
     */
    public void tell(Consumer<T> f) throws InterruptedException {
        c.send(f);
    }

    /**
     * The same as {@link Actor#create(Scope, Object, Consumer)} but with empty close action.
     */
    public static <T> Actor<T> create(Scope scope, T logic) {
     ...
     */
    public static <T> Actor<T> create(Scope scope, T logic, Consumer<T> close) {
        Channel<Consumer<T>> c = Channel.withScopedBufferSize();
        Actor<T> ref = new Actor<>(c);
        scope.fork(() -> {
            try {
                while (true) {
                    Consumer<T> m = c.receive();
                    try {
                        m.accept(logic);
                    } catch (Throwable t) {
                        c.error(t);
    ...
    public static void uninterruptible(Callable<Void> f) throws ExecutionException, InterruptedException {
        unsupervised(scope -> {
            Fork<Void> t = scope.forkUnsupervised(f);

            class JoinDespiteInterrupted {
            ...
            new JoinDespiteInterrupted().join();
            return null;
        });
    }
}
