...
public class Scopes {
     ...
     * f}. Forks can be started using {@link UnsupervisedScope#forkUnsupervised} and {@link
     * UnsupervisedScope#forkCancellable}. All forks are guaranteed to complete before this scope
     * completes.
     *
     * <p>It is advisable to use {@link #supervised(Scoped)} scopes if possible, as they minimise
     * the chances of an error to go unnoticed.
     *
     * <p>The scope is ran in unsupervised mode, that is:
     *
     * <ul>
     *   <li>the scope ends once the {@code f} body completes; this causes any running forks started
     *       within {@code f} to be cancelled
     *   <li>the scope completes (that is, this method returns) only once all forks started by
     *       {@code f} have completed (either successfully, or with an exception)
     *   <li>fork failures aren't handled in any special way, but can be inspected using {@link
     *       Fork#join()}
     * </ul>
     *
     * <p>Upon successful completion, returns the result of evaluating {@code f}. Upon failure, that
     * is an exception thrown by {@code f}, it is re-thrown, wrapped with an {@link
     ...
     * UnsupervisedScope#forkCancellable} or {@link UnsupervisedScope#forkUnsupervised}. All forks
    ...
    public static <T> T supervised(Scoped<T> f) throws InterruptedException {
        ...
        try {
            ...
            // exceptions (InterruptedException or
            // others), but only the first one is propagated below. That's why we add all the other
