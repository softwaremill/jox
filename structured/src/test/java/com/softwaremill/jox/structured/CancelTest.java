...
public class CancelTest {
    ...
    void testCancelBlocksUntilForkCompletes() throws Exception {
        ...
        assertIterableEquals(Arrays.asList("started", "interrupted", "interrupted done", "cancel done"), trail.get());
    }
    ...
    void testCancelBlocksUntilForkCompletesStressTest() throws Exception {
        for (int i = 1; i <= 20; i++) {
            ...
            if (trail.get().size() == 1) {
                assertIterableEquals(List.of("cancel done"), trail.get()); // the fork wasn't even started
            } else {
                assertIterableEquals(Arrays.asList("interrupted", "interrupted done", "cancel done"), trail.get());
            }
        }
    }
