...
class GroupByImpl<T, V, U> {
    ...
    public GroupByImpl(
            Flow<T> parent,
            int parallelism,
            ThrowingFunction<T, V> predicate,
            Flow.ChildFlowTransformer<T, V, U> childFlowTransform) {
        this.parent = parent;
    ...
    private class GroupByState {
        ...
        // A heap with group value (`V`) elements, weighted by the last parent element, mapped to
        // that value.
        ...
        private GroupByState(
                boolean parentDone,
                Optional<PendingFromParent> pendingFromParent,
                Map<V, Channel<T>> children,
                long fromParentCounter,
        ...
        public GroupByState withChildAdded(V v, Channel<T> childChannel) {
            ...
            return new GroupByState(
                    this.parentDone,
                    this.pendingFromParent,
                    newChildren,
                    this.fromParentCounter,
                    this.childMostRecentCounters);
        }
        ...
        public GroupByState withPendingFromParent(T t, V v, long counter) {
            return new GroupByState(
                    this.parentDone,
                    Optional.of(new PendingFromParent(t, v, counter)),
                    this.children,
                    this.fromParentCounter,
                    this.childMostRecentCounters);
        }
        ...
        public Map.Entry<Optional<V>, GroupByState> withoutLongestInactiveChild() {
            Map.Entry<Optional<WeightedHeap.HeapNode<V>>, WeightedHeap<V>> result =
                    this.childMostRecentCounters.extractMin();
            WeightedHeap<V> newHeap = result.getValue();
            return Map.entry(
                    result.getKey().map(WeightedHeap.HeapNode::item),
                    new GroupByState(
                            this.parentDone,
                            this.pendingFromParent,
                            this.children,
                ...
                emit -> {
                    supervised(
                            scope -> {
                                ...
                                // creation (see below). As the receive is conditional, the errors
                                // that occur on this channel are also
                                // propagated to `childOutput`, which is always the first (priority)
                                // clause in the main `select`.
                                Source<FromParent> parentChannel =
                                        parent.map(FromParent::new)
                                                .onError(childOutput::errorOrClosed)
                                                .runToChannel(scope);

                                GroupByState state = new GroupByState();

                                // Main loop; while there are any values to receive (from parent or
                                // children).
                                while (state.elementsCanBeReceived()) {
                                    ...
                                    switch (result) {
                                        case ChannelDone _ -> {
                                            // Only the parent can be done; child completion is
                                            // signalled via a value in `childDone`.
                                            state =
                                                    state.withParentDone(
                                                            parentChannel.isClosedForReceive());
        ...
        if (s.children.containsKey(v)) {
            ...
            scope.forkUnsupervised(
                    () -> {
                        childFlowTransform
                                .apply(v)
                                .apply(Flows.fromSource(childChannel))
                                .onDone(
                                        () -> {
                                            try {
                                ...
                                // When the child flow is done, making sure that the source channel
                                // becomes closed as well
                                // otherwise, we'd be risking a deadlock, if there are
                                // `childChannel.send`-s pending, and the
                                // buffer is full; if the channel is already closed, this is a
                                // no-op.
