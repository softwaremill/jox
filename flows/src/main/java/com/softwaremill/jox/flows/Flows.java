...
public final class Flows {
    ...
    /**
     * Creates a flow, which when run, provides a {@link FlowEmit} instance to the given `withEmit`
     * function. Elements can be emitted to be processed by downstream stages by calling {@link
     * FlowEmit#apply}.
     *
     * <p>The {@link FlowEmit} instance provided to the {@param withEmit} callback should only be
    ...
    /**
     * Creates a flow from the given values. Each value is emitted in order. Flow can be run
     * multiple times.
     */
    @SafeVarargs
    ...
    /**
     * Creates a ByteFlow from given {@link ByteChunk}s. Each ByteChunk is emitted in order. Flow
    ...
    /**
     * Creates a ByteFlow from given byte[]. Each byte[] is emitted in order. Flow can be run
     * multiple times.
     */
    ...
    /**
     * Creates a flow from the given (lazily evaluated) `iterator`. Each element of the iterator is
     * emitted in order. The flow can be run only once, as the iterator is consumed. If you need to
     * run the flow multiple times, use {@link #fromIterator(Supplier)}.
     */
    ...
    /**
     * Creates a flow from the given (lazily evaluated) `iterator`. Each element of the iterator is
     * emitted in order. The flow can be run multiple times, as the `iteratorSupplier` is called
    ...
    /**
     * Creates a flow from the given fork. The flow will emit up to one element, or complete by
     * throwing an exception if the fork fails.
     */
    ...
    /**
     * Creates a flow which emits elements starting with `zero`, and then applying `mappingFunction`
    ...
    /**
     * Creates a flow which emits a range of numbers, from `from`, to `to` (inclusive), stepped by
     * `step`.
     */
    ...
    /**
     * Creates a flow which emits the given `value` repeatedly, at least {@param interval} apart
     * between each two elements. The first value is emitted immediately.
     *
     * <p>The interval is measured between subsequent emissions. Hence, if the following
     * transformation pipeline is slow, the next emission can occur immediately after the previous
     * one is fully processed (if processing took more than the inter-emission interval duration).
     * However, ticks do not accumulate; for example, if processing is slow enough that multiple
                ...
                emit -> {
                    ...
                    while (true) {
                        ...
                        if (sleep > 0) {
                            ...
                            Thread.sleep(
                                    TimeUnit.NANOSECONDS.toMillis(sleep),
                                    (int) (sleep % 1_000_000));
                        }
                    }
                });
    ...
    /**
     * Creates a flow, which emits the result of evaluating `supplierFunction` repeatedly. As the
     * parameter is passed by-name, the evaluation is deferred until the element is emitted, and
    ...
    /**
     * Creates a flow, which emits the value contained in the result of evaluating
     * `supplierFunction` repeatedly. When the evaluation of `supplierFunction` returns a {@link
     * Optional#empty()}, the flow is completed as "done", and no more values are evaluated or
     * emitted.
     *
     * <p>As the `supplierFunction` parameter is passed by-name, the evaluation is deferred until
     * the element is emitted, and happens multiple times.
     *
    ...
    /**
     * Creates a flow which concatenates the given `flows` in order. First elements from the first
     * flow are emitted, then from the second etc. If any of the flows completes with an error, it
     * is propagated.
     */
    @SafeVarargs
    ...
    /**
     * Creates a flow that emits a single element when `from` completes, or throws an exception when
     * `from` fails.
     */
    ...
    /**
     * Creates a flow that emits all elements from the given future {@link Source} when `from`
     * completes.
     *
    ...
    /**
     * Creates a flow which emits the first element ({@link Map.Entry#getKey()}) of entries returned
     * by repeated applications of `f`. The `initial` state is used for the first application, and
     * then the state is updated with the second element of the entry. Emission stops when `f`
     * returns {@link Optional#empty()}, otherwise it continues indefinitely.
     */
    ...
    /**
     * Creates a Flow from a Publisher, that is, which emits the elements received by subscribing to
     * the publisher. A new subscription is created every time this flow is run.
     *
     * <p>The data is passed from a subscription to the flow using a Channel, with a capacity given
     * by the {@link Flow#CHANNEL_BUFFER_SIZE} in scope or {@link Channel#DEFAULT_BUFFER_SIZE} is
     * used. That's also how many elements will be at most requested from the publisher at a time.
     *
     * <p>The publisher parameter should implement the JDK 9+ Flow.Publisher API
     */
                ...
                emit -> {
                    ...
                    supervised(
                            scope -> {
                                ...
                                int capacity =
                                        Flow.CHANNEL_BUFFER_SIZE.orElse(
                                                Channel.DEFAULT_BUFFER_SIZE);
                                int demandThreshold = (int) Math.ceil(capacity / 2.0);

                                // used to "extract" the subscription that is set in the
                                // subscription running in a fork
                                ...
                                try {
                                    // unsafe, but we are sure that this won't throw any exceptions
                                    // (unless there's a bug in the publisher)
                                    ...
                                    while (true) {
                                        ...
                                        if (t instanceof ChannelDone) {
                                            ...
                                            // if we have an element, onSubscribe must have already
                                            // happened; we can read the subscription and cache it
                                            // for later
                                            if (subscription == null) {
                                                subscription = subscriptionRef.get();
                                            }

                                            // now that we've received an element from the channel,
                                            // we can request more
                                            toDemand += 1;
                                    ...
                                    // exceptions might be propagated from the channel, but they
                                    // might also originate from an interruption
                                    return null;
                                } catch (Exception e) {
    ...
    /**
     * Sends a given number of elements (determined by `segmentSize`) from each flow in `flows` to
     * the returned flow and repeats. The order of elements in all flows is preserved.
     *
     * <p>If any of the flows is done before the others, the behavior depends on the `eagerComplete`
     * flag. When set to `true`, the returned flow is completed immediately, otherwise the
     * interleaving continues with the remaining non-completed flows. Once all but one flows are
     * complete, the elements of the remaining non-complete flow are emitted by the returned flow.
     *
     * <p>The provided flows are run concurrently and asynchronously. The size of used buffer is
    ...
    public static <T> Flow<T> interleaveAll(
        ...
        return interleaveAll(
                ...
                segmentSize,
                eagerComplete,
                Flow.CHANNEL_BUFFER_SIZE.orElse(Channel.DEFAULT_BUFFER_SIZE));
    }
                                            ...
                                            () -> {
                                                ...
                                                while (true) {
                                                    ...
                                                    if (received instanceof ChannelDone) {
                                                        ...
                                                        // switch to the next source when
                                                        // segmentSize is reached and there are more
                                                        // sources available
                                                        if (elementsRead == segmentSize
                                                        ...
                                                        //noinspection unchecked
                                                        Object result =
                                                                results.sendOrClosed((T) received);
