    ...
    private void runToSubscriber(Flow.Subscriber<? super T> subscriber) throws ExecutionException, InterruptedException {
        ...
        unsupervised(scope -> {
            ...
            try {
                ...
                Consumer<Throwable> signalErrorAndCancel = e -> {
                    if (!cancelled.get()) {
                        cancel.run();
                        errorSent.set(true);
                ...
                // main processing loop: running as long as flow is not completed or error was received
                while (!cancelled.get()) { // 1.7, 3.12 - ending the main loop after onComplete/onError
                    if (demand.get() == 0) {
                        ...
                        switch (selectOrClosed(errors.receiveClause(), data.receiveClause(), signals.receiveClause())) {
                            ...
                            case ChannelDone _ -> { // only `data` can be done
                                cancel.run(); // 1.6: when signalling onComplete/onError, the subscription is considered cancelled
