...
public class FlowsProjectReactorTest {
    ...
    void simpleFlowShouldEmitElementsToBeProcessedByFlux() throws InterruptedException {
        Scopes.supervised(
                scope -> {
                    ...
                    // when
                    var result =
                            Flux.from(FlowAdapters.toPublisher(flow.toPublisher(scope)))
                                    .map(i -> i * 2)
                                    .collectList()
                                    .block();
    ...
    void concurrentFlowShouldEmitElementsToBeProcessedByFlux() throws InterruptedException {
        Scopes.supervised(
                scope -> {
                    // given
                    var flow =
                            Flows.tick(Duration.ofMillis(100), "x")
    ...
    void shouldCreateFlowFromASimplePublisher() throws Exception {
        // given
        Flux<Integer> map = Flux.fromStream(IntStream.rangeClosed(1, 4).boxed()).map(i -> i * 2);

        // when
        List<Integer> result = Flows.fromPublisher(FlowAdapters.toFlowPublisher(map)).runToList();
    ...
    void shouldCreateFlowFromAConcurrentPublisher() throws Exception {
        ...
        Flux<String> flux =
                Flux.interval(Duration.ofMillis(100))
                        .map(_ -> "x")
                        .mergeWith(Flux.interval(Duration.ofMillis(150)).map(_ -> "y"))
                        .take(5);

        // when
        List<String> result =
                Flows.fromPublisher(FlowAdapters.toFlowPublisher(flux)).map(s -> s + s).runToList();
