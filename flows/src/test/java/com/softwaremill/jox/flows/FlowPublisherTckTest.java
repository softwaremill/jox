...
public class FlowPublisherTckTest {
    ...
    private final FlowPublisherVerification<Integer> verification = new FlowPublisherVerification<>(new TestEnvironment()) {
        @Override
        ...
        public Publisher<Integer> createFailedFlowPublisher() {
            return Flows.<Integer>failed(new RuntimeException("boom")).toPublisher(scope.get());
        }
    };
    ...
    private static void handleInvocationTargetException(Method method, Throwable e) {
        ...
        String errorMessage = String.format("Error in method %s:%n%s%n%s",
                method.getName(),
                cause.getMessage(),
                getStackTrace(cause));
