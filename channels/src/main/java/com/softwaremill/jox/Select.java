...
public class Select {
    ...
    */
     ...
     *
     * <p>If a couple of the clauses can be completed immediately, the select is biased towards the
     * clauses that appear first.
     *
    ...
    public static <U> Object selectOrClosed(SelectClause<? extends U>... clauses)
        ...
        while (true) {
            ...
            if (r == RestartSelectMarker.RESTART) {
                // in case a `CollectSource` function filters out the element (the transformation
                // function returns `null`,
                // which is represented as a marker because `null` is a valid result of
                // `doSelectorClosed`, e.g. for send clauses),
    ...
    /**
     * Select exactly one clause to complete, with a timeout. Each clause should be created for a
     * different channel.
     *
     * <p>If a couple of the clauses can be completed immediately, the select is biased towards the
     * clauses that appear first.
     *
     * <p>If no clauses are given, throws {@link IllegalArgumentException}.
     *
     * <p>If the timeout elapses before any clause can be selected, throws {@link TimeoutException}.
     *
    ...
    /**
     * Select exactly one clause to complete, with a timeout. Each clause should be created for a
     * different channel. Doesn't throw exceptions when the channel is closed or timeout occurs, but
     * returns a value.
     *
     * <p>If a couple of the clauses can be completed immediately, the select is biased towards the
     * clauses that appear first.
     *
     * <p>If no clauses are given, returns the timeout value.
     *
     ...
     */
    @SafeVarargs
    public static <U> Object selectOrClosedWithin(
            Duration timeout, U timeoutValue, SelectClause<? extends U>... clauses)
            throws InterruptedException {
                        ...
                        .start(
                                () -> {
                                    try {
                                        ...
                                        // this might happen - ignore, since we don't want the
                                        // uncaught exception handler to be called
                                    }
                                });

        // Create a new array with the timeout clause appended
        @SuppressWarnings("unchecked")
        SelectClause<Object>[] clausesWithTimeout =
                Arrays.copyOf(clauses, clauses.length + 1, SelectClause[].class);
    ...
    private static <U> Object doSelectOrClosed(SelectClause<? extends U>... clauses)
        ...
        // check that the clause doesn't refer to a channel that is already used in a different
        // clause
        ...
        for (int i = 0; i < clauses.length; i++) {
            ...
            if (!si.register(clause)) {
                break; // channel is closed, or a clause was selected - in both cases, no point in
    ...
    private static boolean verifyChannelsUnique_getAreAllRendezvous(SelectClause<?>[] clauses) {
        ...
        // we expect the number of clauses to be small, so that this n^2 double-loop is faster than
        // allocating a set
        for (int i = 0; i < clauses.length; i++) {
...
class SelectInstance {
    ...
    static {
        try {
            MethodHandles.Lookup l =
                    MethodHandles.privateLookupIn(SelectInstance.class, MethodHandles.lookup());
    ...
    /**
     * The content of the list will be written & read only by the main select thread. Hence, no
    ...
    /**
     * Register a clause in this select instance. Only one clause for each channel should be
     * registered.
     *
    ...
    <U> boolean register(SelectClause<U> clause) {
        ...
        if (result instanceof StoredSelectClause ss) {
            ...
            // - a list of clauses to re-register - there's no point in doing that anyway (since the
            // channel is closed)
            ...
            // when setting the state, we might override another state:
            // - a list of clauses to re-register - there's no point in doing that anyway (since we
            // already selected a clause)
            // - a closed state - the closure must have happened concurrently with registration; we
            // give priority to immediate selects then
            state = clause;
            return false;
        }
    }
            ...
            if (currentState == SelectState.REGISTERING) {
                        ...
                        if (spinIterations > 0) {
                            ...
                            if (Thread.interrupted()) {
                                if (STATE.compareAndSet(
                                        this, currentThread, SelectState.INTERRUPTED)) {
                                    // since we changed the state, we know that none of the clauses
                ...
                if (STATE.compareAndSet(this, currentState, SelectState.REGISTERING)) {
                    ...
                    for (var clause : (List<SelectClause<?>>) currentState) {
                        // cleaning up & removing the stored select for the clause which we'll
                        // re-register
                ...
                // clause selected during registration - result in
                // `resultSelectedDuringRegistration`

                cleanup(selectedClause);

                // running the transformation at the end, after the cleanup is done, in case this
        ...
        while (true) {
            ...
            if (currentState == SelectState.REGISTERING) {
                if (STATE.compareAndSet(
                        this,
                        currentState,
                        Collections.singletonList(storedSelectClause.getClause()))) {
                ...
                var newClausesToReRegister =
                        new ArrayList<SelectClause<?>>(clausesToReRegister.size() + 1);
                //noinspection unchecked
                newClausesToReRegister.addAll(
                        (Collection<? extends SelectClause<?>>) clausesToReRegister);
    ...
    boolean channelClosed(ChannelClosed channelClosed) {
        while (true) {
            var currentState = state;
            if (currentState == SelectState.REGISTERING) {
                // the channel closed state will be discovered when there's a call to
                // `checkStateAndWait` after registration completes
